...
class VeterinaryControllerAppointmentTest {
    ...
    Pet pet = new Pet();
    Appointment updateAppointment = new Appointment(1L, LocalDateTime.of(2025, 8, 5, 12, 0), "Routine visit", "General Check-up", false, pet);

    doNothing().when(veterinaryServiceAppointment).updateAppointment(updateAppointment,1L);

    mockMvc.perform(put("/appointment/1")
                    .contentType("application/json")
                    .content("{\"id\":1,\"dateTime\":\"05/08/2024 12:00\",\"reason\":\"Routine visit\",\"consultationType\":\"General Check-up\",\"status\":Scheduled,\"pet\":{}}"))
            .andExpect(status().isOk());
    }
