...
class VeterinaryControllerPetTest {
    ...
    void testUpdatePet() throws Exception {
        ...
        updatePet.setId(3L);
        updatePet.setName("Patri");
        updatePet.setAge(5);
        updatePet.setBreed("Mutt");
        updatePet.setGender("Male");
    
        doNothing().when(veterinaryServicePet).updatePet(updatePet,3L);

        mockMvc.perform(put("/pets/3"))
                        .contentType("application/json")
                        .content("{\"id\":3,\"name\":\"Patri\",\"age\":5,\"breed\":\"Mutt\",\"gender\":\"Male\"}")
                .andExpect(status().isOk());
